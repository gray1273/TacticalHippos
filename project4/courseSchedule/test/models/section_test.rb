...
class SectionTest < ActiveSupport::TestCase
  def test_accessing_nested_data
  	...
  	assert_equal 0, testSection.course_id
  	assert_equal "Instructor", testSection.user.user_type.title
  	assert_equal "fn", testSection.user.first_name
  	assert_equal "ln", testSection.user.last_name
  	assert_equal "instructor@mail.com", testSection.user.email
  ...
  def test_activeRecord
  	user_type = UserType.find_or_create_by(:title => "Instructor")
  ...
  def test_createSafely
  	...
  	testSection = controller.create_safely(0, 0010, 12345, "fn", "ln", "Lecture", "Autumn 2021", "Distance Learning", "Online", 2021, 7, 1, 2021, 12, 20, 12, 45, 14, 5)

  	assert_equal 0, testSection.course_id
  	assert_equal "Instructor", testSection.user.user_type.title
  	assert_equal "fn", testSection.user.first_name
  	assert_equal "ln", testSection.user.last_name
  	assert_equal "", testSection.user.email
  	assert_equal "Lecture", testSection.section_type.title
  	assert_equal "Autumn", testSection.term.season.title
  	assert_equal 2021, testSection.term.year
  	assert_equal "Distance Learning", testSection.instruction_mode.mode
